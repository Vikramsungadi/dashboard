...
const Layout = (props: Props) => {
	...
	let totalPages = Math.floor(totalPhotos / 20 - 1);
	...
	useLayoutEffect(() => {
		axios
			...
			.then((res: AxiosResponse<TPhoto[], any[]>) => setTotalPhotos(res.data.length));
	}, []);
		...
		<>
			...
			<nav className='mx-auto mt-10'>
				<ul className='flex gap-2.5 '>
					<PaginationItem href={`/photos/${1}`} className={cn("ml-4 px-4", isFirstPage && disabledClasses)}>
						<Rewind />
					</PaginationItem>
					<PaginationItem
						href={isFirstPage ? `/photos/${currentPage}` : `/photos/${currentPage - 1}`}
					...
					{currentPage + 3 < totalPages && (
						<PaginationItem href={`/photos/${totalPages}`} className={cn("size-10")}>
							{totalPages}
						</PaginationItem>
					)}

					<PaginationItem
						href={isLastPage ? `/photos/${currentPage}` : `/photos/${currentPage + 1}`}
						className={cn("mr-4 px-4", isLastPage && disabledClasses)}>
						Next
					</PaginationItem>
					<PaginationItem
						href={`/photos/${totalPages}`}
